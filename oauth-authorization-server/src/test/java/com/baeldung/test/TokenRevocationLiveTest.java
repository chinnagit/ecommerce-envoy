...
public class TokenRevocationLiveTest {
    ...
    public void cartServiceTest() {
        ...
        final Response resourceServerResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken).get("http://localhost:5222/cart-service/cart");
    ...
    private Response obtainAccessToken(String clientId, String username, String password) {
        ...
        return RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:9081/spring-security-oauth-server/oauth/token");
    ...
    private String obtainRefreshToken(String clientId, final String refreshToken) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:9081/spring-security-oauth-server/oauth/token");
    ...
    private void authorizeClient(String clientId) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:9081/spring-security-oauth-server/oauth/authorize");
    }
}